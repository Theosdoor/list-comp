...
If you want to use Weights & Biases for SAE training, create a local environment file:
...
- `scripts/`: runnable SAE training and analysis entry points.
...
Generate aggregate plots and markdown reports for the relationship between SAE quality metrics and attention-correlated special features:
...
The script accepts one or more SAE directories or `.pt` checkpoint files via `--sae_dirs`. It loads the transformer, computes reconstruction and downstream metrics for each SAE, detects special features using the SEP attention-difference correlation threshold, and writes plots, CSV metrics, and markdown summaries under `--output_dir`.
